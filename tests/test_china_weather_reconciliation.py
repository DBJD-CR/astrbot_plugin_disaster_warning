...
def _require_feature(test_case: unittest.TestCase, value: object, name: str):
    if value is None:
...
class ChinaWeatherReconciliationTests(unittest.TestCase):
    ...
    def test_weather_parser_retains_id_after_32_later_events(self) -> None:
        ...
        self.assertIsNotNone(parser._parse_data(self._weather_payload(first_id)))
...
class ChinaWeatherReconcilerAsyncTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_escaped_dispatch_exception_is_consumed_at_most_once(self) -> None:
        ...
        self.assertEqual(getattr(failed_cycle, "consumed_count", None), 1)
        self.assertEqual(failed_cycle.failed_identifiers, ())
        self.assertEqual(failed_cycle.consumed_error_identifiers, (WARNING_ID_B,))
